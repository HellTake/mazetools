...
class maze:
    ...
    def create_bround(self,x,y):
        for i in range(self.width+2):
            self.create_block(x+i*2,y,'green')
            self.create_block(x+i*2,y-(self.height+1)*2,'green')
        for i in range(self.height+2):
            self.create_block(x,y-i*2,'green')
            self.create_block(x+(self.width+1)*2,y-i*2,'green')
